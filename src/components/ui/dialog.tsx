...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("sticky top-0 z-10 bg-background pb-4 flex flex-col space-y-1.5 text-center sm:text-left -mx-6 -mt-6 px-6 pt-6 border-b border-border", className)} {...props} />
);
DialogHeader.displayName = "DialogHeader";

const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("sticky bottom-0 z-10 bg-background pt-4 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 -mx-6 -mb-6 px-6 pb-6 border-t border-border", className)} {...props} />
);
