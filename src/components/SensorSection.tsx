...
const SensorSection = ({ content }: { content?: any }) => {
  const title = content?.title || "The Sensor";
    ...
    <section id="Sensor" className="relative pt-[calc(1.6rem+96px)] md:pt-[calc(2.4rem+144px)] pb-[51px] lg:pb-[77px] mt-[120px] md:mt-[180px] text-white" style={{ backgroundColor: "#000000" }}>
      ...
      <div className="absolute top-0 left-0 right-0 z-10 h-[120px] md:h-[180px] -translate-y-[calc(100%-1px)]">
        <svg viewBox="0 0 1440 180" preserveAspectRatio="none" className="absolute bottom-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <path d="M0,90 C200,130 400,170 600,160 C800,150 950,20 1100,10 C1250,0 1380,40 1440,70 L1440,180 L0,180 Z" fill="#000000" />
        </svg>
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-10 text-center">
        <ScrollReveal>
          ...
          <h2 className="text-[40px] md:text-[60px] lg:text-[80px] font-semibold mb-8 leading-[1.05]">{title}</h2>
        </ScrollReveal>

        <ScrollReveal delay={0.1}>
          <div className="max-w-4xl mx-auto mb-[64px]">
            {paragraphs.map((p: string, i: number) => (
              <p key={i} className="text-maxir-white text-base leading-relaxed mb-4 last:mb-0">{p}</p>
            ))}
          </div>
        </ScrollReveal>
