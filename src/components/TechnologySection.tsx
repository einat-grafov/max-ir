...
const DEFAULT_SUBTITLE = "Max-IR's patented technology unlocks the plethora of information that exists in the infrared spectra but is difficult to analyze.";
...
const TechnologySection = ({ content }: { content?: any }) => {
  const title = content?.title || "The Technology";
  const subtitle = content?.subtitle || DEFAULT_SUBTITLE;
  const paragraphs: string[] = content?.paragraphs || DEFAULT_PARAGRAPHS;
  const diagramImage = content?.diagram_image || "/images/diagram.gif";
    ...
    <section id="Technology" className="section-white py-16 lg:py-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div>
            <ScrollReveal>
              ...
              <h2 className="text-[40px] md:text-[60px] lg:text-[80px] font-semibold text-foreground leading-[1.05] mb-8">
                {title.includes(" ") ? (
                  <>
                    {title.split(" ").slice(0, -1).join(" ")}<br />{title.split(" ").slice(-1)[0]}
                  </>
                ) : title}
              </h2>
            </ScrollReveal>
          ...
          <div className="flex flex-col gap-6">
            {paragraphs.map((p: string, i: number) => (
              <ScrollReveal key={i} delay={0.15 + i * 0.05}>
                <p className="text-foreground text-[16px] lg:text-[18px] leading-relaxed">{p}</p>
              </ScrollReveal>
            ))}
          </div>
        </div>
      ...
      <ScrollReveal variant="scaleIn">
        <div className="w-full mt-12 lg:mt-16 mb-16">
          <img src={diagramImage} alt="MaxIR Diagram" className="w-full" />
        </div>
      </ScrollReveal>

      <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
        {/* Feature icons */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 lg:gap-6 items-start">
