...
import { Briefcase, MessageSquare, Plus, Pencil, Trash2 } from "lucide-react";
import IconTooltipButton from "@/components/admin/IconTooltipButton";
import RecordCareerInteractionModal from "@/components/admin/RecordCareerInteractionModal";
import {
...
const iconElements: Record<TimelineEvent["type"], React.ReactNode> = {
  applied: <Briefcase className="h-2 w-2 text-white" />,
  note: <MessageSquare className="h-2 w-2 text-white" />,
};

const CareerApplicationTimeline = ({ applicationId, applicantName, applicationCreatedAt }: Props) => {
  const queryClient = useQueryClient();
  const [modalOpen, setModalOpen] = useState(false);
  const [editingNote, setEditingNote] = useState<CareerNote | null>(null);
  const [deletingNote, setDeletingNote] = useState<CareerNote | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("career_application_notes").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
...
export default CareerApplicationTimeline;
