...
type CareerNote = Tables<"career_application_notes">;

interface RecordCareerInteractionModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  applicationId: string;
  applicantName: string;
  editNote?: CareerNote | null;
}

const RecordCareerInteractionModal = ({
  open,
  onOpenChange,
  applicationId,
  applicantName,
  editNote,
}: RecordCareerInteractionModalProps) => {
  const queryClient = useQueryClient();
  const isEditing = !!editNote;

  const [dateOfInteraction, setDateOfInteraction] = useState<Date>(new Date());
  const [contact, setContact] = useState(applicantName);
  ...
  useEffect(() => {
    if (open && editNote) {
      ...
      setContact(editNote.contact_person || applicantName);
      ...
      setFeedback(editNote.customer_feedback || "");
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEditing && editNote) {
        ...
        const { error } = await supabase.from("career_application_notes").insert(noteData as any);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["career-application-notes", applicationId] });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[640px] max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="shrink-0 border-b border-border pb-4">
          <DialogTitle>{isEditing ? "Edit Interaction" : "Record Applicant Interaction"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5 overflow-y-auto flex-1 pt-4">
            ...
            <div>
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar mode="single" selected={dateOfInteraction} onSelect={(d) => d && setDateOfInteraction(d)} initialFocus className={cn("p-3 pointer-events-auto")} />
                </PopoverContent>
              </Popover>
            </div>
            <div>
          ...
          {interactionType === "Other" && (
            <div>
              ...
              <Input value={interactionTypeOther} onChange={(e) => setInteractionTypeOther(e.target.value)} placeholder="e.g. Interview, Reference check" className="mt-1.5" />
            </div>
          )}
          ...
          <div>
            ...
            <Textarea value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Enter key points discussed" className="mt-1.5 min-h-[80px] resize-none" />
          </div>

          <div>
            <Label className="text-sm font-medium">Action Items / Next Steps</Label>
            <Textarea value={actionItems} onChange={(e) => setActionItems(e.target.value)} placeholder="Enter next steps or follow-up tasks" className="mt-1.5 min-h-[60px] resize-none" />
          </div>

          <div>
            <Label className="text-sm font-medium">Applicant Feedback / Notes</Label>
            <Textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder="Insert any feedback or notes from the applicant." className="mt-1.5 min-h-[60px] resize-none" />
          </div>
          ...
          <div className="border border-border rounded-lg p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="text-xs max-w-[200px]">Check this if the applicant needs a follow-up such as an interview or reply.</p>
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="flex items-center gap-2">
              <Checkbox checked={followUpRequired} onCheckedChange={(c) => setFollowUpRequired(c === true)} id="career-follow-up-yes" />
              <label htmlFor="career-follow-up-yes" className="text-sm">Yes, follow-up needed</label>
            </div>
            {followUpRequired && (
              <div className="space-y-3">
                <div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className={cn("w-full justify-start text-left font-normal mt-1.5", !nextFollowUpDate && "text-muted-foreground")}>
                        <CalendarIcon className="h-4 w-4 mr-2" />
                        {nextFollowUpDate ? format(nextFollowUpDate, "PPP") : "Pick a date"}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar mode="single" selected={nextFollowUpDate} onSelect={setNextFollowUpDate} initialFocus className={cn("p-3 pointer-events-auto")} />
                    </PopoverContent>
                  </Popover>
                </div>
                <div>
                  <Label className="text-sm font-medium">Follow-up Details</Label>
                  <Input value={followUpDetails} onChange={(e) => setFollowUpDetails(e.target.value)} placeholder="Describe the follow-up action needed" className="mt-1.5" />
                </div>
              </div>
            )}
          </div>
...
export default RecordCareerInteractionModal;
